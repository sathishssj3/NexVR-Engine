#!/usr/bin/env node
/**
 * Release Broadcast System — Stereo Engine
 * 
 * Usage:
 *   node scripts/send-release-broadcast.mjs --preview
 *   node scripts/send-release-broadcast.mjs --dry-run
 *   node scripts/send-release-broadcast.mjs --test-to=you@gmail.com
 *   node scripts/send-release-broadcast.mjs --send
 */
...
const testArg = process.argv.includes('--test');
const apiKeyArg = process.argv.find(arg => arg.startsWith('--api-key='));

let testEmail = testToArg ? testToArg.split('=')[1].trim() : null;
const cliApiKey = apiKeyArg ? apiKeyArg.split('=')[1].trim() : null;
const effectiveApiKey = cliApiKey || process.env.RESEND_API_KEY;
...
function getSubscribers() {
  ...
  console.log('Exporting waitlist from Cloudflare KV...');
  execSync('node scripts/export-waitlist.mjs', { cwd: rootDir, stdio: 'inherit' });
  return JSON.parse(fs.readFileSync(jsonPath, 'utf8'));
}

/**
 * 1:1 Landing Page Aesthetic HTML Email Template
 * - Uses Chakra Petch display font + Inter body font
 * - Clean "STEREO ENGINE" wordmark (NO logo icon)
 * - Symmetrical crimson red wings
 * - Cockpit 3-column instrument stat rail (3 APIs | 6 Profiles | 11.1ms Budget)
 * - Anaglyph stereo red shadow title
 * - Landing-page style Founder Pass & Tagpills
 */
function buildEmailHtml(recipientEmail) {
  return `<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="utf-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <title>${CONFIG.appName} is Available</title>
    ...
    <tr>
      <td align="center" style="padding: 24px 12px 60px;">
        ...
        <table role="presentation" width="100%" border="0" cellspacing="0" cellpadding="0" class="email-container" style="max-width: 600px; background-color: #0C0C0F; border: 1px solid rgba(255, 255, 255, 0.08); border-radius: 2px; overflow: hidden;">
          ...
          <tr>
            <td style="padding: 38px 28px 24px; text-align: center;">
              ...
              <p style="font-size: 15px; line-height: 1.65; color: #7A7A80; margin: 0 auto 30px; max-width: 470px;">
                Turn your entire flat PC library into immersive 6DOF VR with a single click. <strong style="color: #FFFFFF;">${CONFIG.appName}</strong> hooks the graphics pipeline while your game runs, reconstructs real-time stereo depth, and hands it straight to your OpenXR headset.
              </p>
          ...
          <!-- Shipped Profiles Section (Matching Landing Page Cards & Tagpills) -->
          <tr>
            <td style="padding: 0 24px 28px;">
          ...
          <!-- Quick 3-Step Setup (Clean Dark Box with Hover Effect) -->
          <tr>
          ...
          <!-- Footer (High-Tech, Clean, Perfectly Placed) -->
          <tr>
            <td style="background-color: #08080B; border-top: 1px solid rgba(255, 255, 255, 0.06); padding: 34px 24px 30px; text-align: center;">
              ...
              <div style="font-family: 'Chakra Petch', sans-serif; font-size: 10px; font-weight: 600; letter-spacing: 0.16em; text-transform: uppercase; margin-bottom: 18px;">
                <a href="${CONFIG.websiteUrl}" class="footer-link footer-nav-link" style="color: #8A8A92 !important; text-decoration: none; padding: 4px 10px;"><font color="#8A8A92" style="color: #8A8A92 !important;">PORTAL</font></a>
              ...
              <div style="font-size: 10px; line-height: 1.6; color: #5C5C64; letter-spacing: 0.03em;">
                Single-player universal VR injector · © 2026 ${CONFIG.companyName} · Bengaluru, KA / Global<br>
                Early access notification for registered waitlist · <a href="${CONFIG.websiteUrl}#unsubscribe" class="footer-link" style="color: #7A7A80; text-decoration: underline;">Unsubscribe</a>
              </div>
...
function buildPlainText(recipientEmail) {
  return `STEREO ENGINE — Official Engine Launch ${CONFIG.version}

ONE CLICK. ANY GAME IN VR.
Turn your entire flat PC library into immersive 6DOF VR with a single click. Stereo Engine hooks the graphics pipeline while your game runs, reconstructs real-time stereo depth, and hands it straight to your OpenXR headset.

Download Stereo Engine (${CONFIG.version}):
${CONFIG.downloadUrl}
...
Portal: ${CONFIG.websiteUrl}
Troubleshooting: ${CONFIG.websiteUrl}#faq

Sent to ${recipientEmail} because you joined early access.
Single-player universal VR injector. © 2026 ${CONFIG.companyName}.`;
}

async function main() {
  console.log(`\n======================================================`);
  console.log(`   ${CONFIG.appName.toUpperCase()} — Automated Release Broadcast`);
  console.log(`======================================================\n`);

  // Preview Mode
  if (isPreview) {
    const previewPath = path.join(rootDir, 'release-email-preview.html');
    fs.writeFileSync(previewPath, buildEmailHtml('customer@example.com'), 'utf8');
    console.log(`[SUCCESS] Landing-page style HTML email preview generated at:\n  ${previewPath}\n`);
    console.log(`Open this file in your browser to inspect the design!`);
    return;
  }

  // Single Test Send Mode
  if (testEmail || testArg) {
    if (!testEmail) {
      const subscribers = getSubscribers();
      if (subscribers.length > 0) {
        testEmail = subscribers[0].email;
        console.log(`No --test-to provided. Defaulting to first registered subscriber: ${testEmail}`);
      } else {
        console.error(`Please provide an email to test: --test-to=you@gmail.com`);
        process.exit(1);
      }
    }

    console.log(`[TEST MODE] Sending test release email to: ${testEmail}`);
    const apiKey = effectiveApiKey;
    if (!apiKey) {
      console.error(`\n[ERROR] Resend API Key is required to send real emails.`);
      console.log(`\nHOW TO GET YOUR FREE API KEY (takes 1 minute):`);
      console.log(`  1. Go to https://resend.com and sign up with ${testEmail}`);
      console.log(`  2. Click "API Keys" -> "Create API Key" (starts with "re_")`);
      console.log(`  3. Run with your key:`);
      console.log(`     npm run broadcast:test -- --test-to=${testEmail} --api-key=re_your_api_key`);
      console.log(`     OR set it in your .env file: RESEND_API_KEY=re_your_api_key\n`);
      process.exit(1);
    }
    ...
    try {
      const res = await fetch('https://api.resend.com/emails', {
        ...
        body: JSON.stringify({
          ...
          html: buildEmailHtml(testEmail),
          text: buildPlainText(testEmail),
          headers: {
            'List-Unsubscribe': `<mailto:unsubscribe@resend.dev?subject=unsubscribe>, <${CONFIG.websiteUrl}#unsubscribe>`,
      ...
      if (res.ok) {
        ...
        console.log(`\n======================================================`);
        console.log(`[SUCCESS] Test email successfully delivered to ${testEmail}!`);
        console.log(`Resend Message ID: ${data.id}`);
        console.log(`Check your inbox at ${testEmail} to inspect the live email.`);
        console.log(`======================================================\n`);
      } else {
        const errText = await res.text();
        console.error(`\n[FAILED] Resend API error: ${errText}`);
        if (errText.includes('testing emails') || errText.includes('only send testing')) {
          console.log(`\n[TIP] On Resend's free tier with 'onboarding@resend.dev', you can send test emails to the exact email address you signed up with at resend.com.`);
        }
      }
    } catch (err) {
  ...
  const subscribers = getSubscribers();
  console.log(`Total waitlist customers registered: ${subscribers.length}`);

  // Dry-Run Mode (Default safe mode)
  if (isDryRun || !isSend) {
    console.log(`\n[DRY RUN SAFE MODE] No emails were sent.`);
    console.log(`The following ${subscribers.length} customer(s) will receive the release email:\n`);
    subscribers.forEach((s, idx) => console.log(`  ${idx + 1}. ${s.email} (${s.country})`));
    console.log(`\nNext Steps:`);
    console.log(`  • Preview HTML in browser:     npm run broadcast:preview`);
    console.log(`  • Send test email to yourself: npm run broadcast:test -- --test-to=${subscribers[0]?.email || 'your@email.com'}`);
    console.log(`  • Broadcast to all customers:  npm run broadcast:send\n`);
    return;
  }

  // Full Live Broadcast Mode
  const apiKey = effectiveApiKey;
  if (!apiKey) {
    console.error(`\n[ERROR] RESEND_API_KEY environment variable is required.`);
    console.log(`Get a free API key at https://resend.com (3,000 free emails/month).`);
    console.log(`Then set it in PowerShell:`);
    console.log(`  $env:RESEND_API_KEY="re_your_api_key"`);
    console.log(`  npm run broadcast:send\n`);
    process.exit(1);
  }

  console.log(`\nInitiating live release broadcast to all ${subscribers.length} customers...\n`);
  ...
  console.log(`\n======================================================`);
  console.log(`Broadcast Complete: ${sent} delivered, ${failed} failed.`);
  console.log(`======================================================\n`);
}
