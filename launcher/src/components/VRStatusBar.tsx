  ...
  return (
    <div className="glass-panel" style={{ height: 76, borderTop: '1px solid var(--ag-border)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 40px', zIndex: 10, borderBottom: 'none', borderLeft: 'none', borderRight: 'none', boxShadow: '0 -10px 40px rgba(0,0,0,0.5), inset 0 1px 1px rgba(255,255,255,0.06)' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 15 }}>
        <button onClick={() => { if(selectedGame) window.ag.utils.openConfig(selectedGame.id); }} style={{ background: 'transparent', border: 'none', color: 'var(--ag-text-muted)', cursor: 'pointer', fontFamily: 'var(--ag-font-mono)', fontSize: 12, letterSpacing: '1px' }}>[ CONFIG ]</button>
        <button onClick={() => { if(selectedGame) window.ag.utils.openLog(selectedGame.id); }} style={{ background: 'transparent', border: 'none', color: 'var(--ag-text-muted)', cursor: 'pointer', fontFamily: 'var(--ag-font-mono)', fontSize: 12, letterSpacing: '1px' }}>[ LOGS ]</button>
        ...
        <button 
          ...
          style={{ 
            background: (injectState === 'injecting' || injectState === 'running') ? 'rgba(255,0,60,0.15)' : injectState === 'success' ? 'rgba(0,255,136,0.15)' : 'rgba(0,240,255,0.08)', 
