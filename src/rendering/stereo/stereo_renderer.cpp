...
bool StereoRenderer::RenderStereoFrame(ID3D11DeviceContext* context,
    ...
    bool useASW = (frameCtx.constants.enableASW != 0) && aswManager_.ShouldSynthesizeFrame(frameCtx.constants.aswTargetFps);

    if (useASW) {
        aswManager_.OnSyntheticFrameSubmitted();
        state_ = StereoRendererState::ASW_ACTIVE;
    } else {
        aswManager_.OnRealFrameSubmitted();
    ...
    // 2. Bind State
    ID3D11ComputeShader* cs = useASW ? resourceManager->GetAswShader() : resourceManager->GetReprojectionShader();
    if (!cs) {
