...
- **Fix**: Changed the system-wide default for `srgbCorrection` to `false` in `ConfigManager`, schemas, and overlay reset handlers. Updated all curated profiles (`990080_hogwarts_legacy.json`, `668580_atomic_heart.json`, `1623730_palworld.json`, `1245620_elden_ring.json`, `1110910_mortal_shell.json`, `meccha_chameleon.json`, `814380_sekiro.json`) to `srgbCorrection: false`. Added a hardened launcher fallback in `injectionManager.ts` for Hogwarts Legacy (`Phoenix` / `HogwartsLegacy`). Reprojection shaders now faithfully pass through untouched backbuffer colors directly into the OpenXR eye swapchains, matching desktop monitor lighting, colors, and shadow gradients 1:1 across all tested and future games.
- **Rule**: Universal default for `srgbCorrection` must remain `false`. Games output already-tonemapped sRGB frames; compute shaders must never artificially apply exponential gamma curves (`pow(c, 2.2)`) unless specifically requested by an opt-in user toggle.
